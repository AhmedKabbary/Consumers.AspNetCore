...
Route ASP.NET Core endpoints based on consumer type.

## Motivation

Sometimes you need to perform different logic or return different data to different API consumers using the same route.

For example, same route returns different data for users and admins.

You can do this by branching the logic using control statements (if) or create different routes for each consumer.

`Consumers.AspNetCore` helps you solve this problem in a clean and declarative way by separating the endpoints (with the same method/path combination) and routes between them based on consumer type passed in a custom header.
...
builder.Services.AddConsumers(options =>
{
    ...
    options.Consumers.Add("Vendor");
    options.Consumers.Add("Customer");
});
```
